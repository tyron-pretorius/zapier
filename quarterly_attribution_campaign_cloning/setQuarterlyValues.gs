function setQuarterlyValues() {
  ...
  if (mm%3 == 1)
  {
    ...
    column = data.indexOf('Quarterly') + 1;//Arrays are zero indexed- add 1
    
    var lastRow = sheet.getLastRow();
    var values = sheet.getRange(1,column, lastRow).getValues();
    var length = values.filter(String).length;
    ...
    var quarters = submissions.getRange(row, 3);
    quarters.setValue(string);
    Logger.log(ts);
